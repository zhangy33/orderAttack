...
public class SUNDR implements Serializable, NewProtocol  {
	...
	public byte[] updateServer(String o, byte[] packUpdateServer, byte[] serverDataBase, List<PublicKey> pubKeys) {
		try {
			...
			// attack!!!
			VSstruct attack = (VSstruct) SDK.deserialize(serverDataBase_local.VSL.get(1));
			attack.pairs[1] = 100;
			byte[] attack_byte = SDK.serialize(attack); 
			serverDataBase_local.VSL.set(1, attack_byte);
